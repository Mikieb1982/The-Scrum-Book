...
export const MatchDetailView: FC<MatchDetailViewProps> = ({ match, isAttended, onBack, onToggleAttendance }) => {
  if (!match) {
    return (
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className={`rounded-full px-4 py-2 text-sm font-semibold uppercase tracking-wide transition ${
            isAttended ? 'bg-orange-500 text-white hover:bg-orange-600' : 'bg-slate-900 text-white hover:bg-slate-800'
          }`}
        >
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <Tile title="Fixture" eyebrow={match.round} highlight>
          ...
          <p className="text-base text-slate-600 dark:text-slate-200">{formattedDate} · Kick-off {match.kickoffTime}</p>
        ...
        <Tile title="Venue" eyebrow={match.venue.city}>
          ...
          {match.venue.capacity && (
            <p className="mt-3 text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
              Capacity {match.venue.capacity.toLocaleString()} supporters
            </p>
          )}
        </Tile>
